    ...
    _progressIndi(new QProgressIndicator (this))
{
    ...
    setSubTitle(WizardCommon::subTitleTemplate().arg(tr("Cài đặt %1 máy chủ").arg(theme->appNameGUI())));
...
void OwncloudSetupPage::slotUrlChanged(const QString& url)
{
    ...
    if (!url.startsWith(QLatin1String("https://"))) {
        ...
        _ui.urlLabel->setToolTip(tr("Url này không an toàn vì nó không được mật mã.\n"
                                    "Không nên sử dụng nó."));
    } else {
        _ui.urlLabel->setPixmap(QPixmap(Theme::hidpiFileName(":/client/resources/lock-https.png")));
        _ui.urlLabel->setToolTip(tr("Url này là an toàn. Bạn co thể sử dụng nó."));
    }
}
...
void OwncloudSetupPage::initializePage()
{
    ...
    if (Theme::instance()->overrideServerUrl().isEmpty()) {
        ...
        setButtonText(QWizard::CommitButton, tr("&Tiếp tục >"));
        validatePage();
        setVisible(false);
    }
}
