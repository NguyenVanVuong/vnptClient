    ...
    _remoteFolder()
{
    ...
    setTitle(WizardCommon::titleTemplate().arg(tr("Kết nối tới %1").arg(theme->appNameGUI())));
    setSubTitle(WizardCommon::subTitleTemplate().arg(tr("Thiết lập các tùy chọn thư mục trên máy tính")));
    ...
    setButtonText(QWizard::NextButton, tr("Kết nối..."));
...
void OwncloudAdvancedSetupPage::updateStatus()
{
    ...
    if (dataChanged()) {
        if( _remoteFolder.isEmpty() || _remoteFolder == QLatin1String("/") ) {
            ...
            t = tr("%1 thư mục '%2' đã được đồng bộ tới thư mục trên máy tính '%3'")
                .arg(Theme::instance()->appName()).arg(_remoteFolder)
                .arg(QDir::toNativeSeparators(locFolder));
              _ui.rSyncEverything->setText(tr("Đồng bộ thư mục '%1'").arg(_remoteFolder));
        }

        const bool dirNotEmpty(QDir(locFolder).entryList(QDir::AllEntries | QDir::NoDotAndDotDot).count() > 0);
        if(dirNotEmpty) {
            t += tr("<p><small><strong>Cảnh báo:</strong> Thư mục trên máy tính không rỗng. "
                    "Chọn một giải pháp khác!</small></p>");
        }
