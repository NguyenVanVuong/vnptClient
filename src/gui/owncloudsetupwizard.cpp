...
{
    _ocWizard->displayError(tr("Thất bại khi kết nối với %1 tại %2:<br/>%3")
                            .arg(Theme::instance()->appNameGUI(),
                                 reply->url().toString(),
...
{
    _ocWizard->displayError(tr("Quá thời gian khi kết nối với %1 tại %2.")
                            .arg(Theme::instance()->appNameGUI(),
                                 url.toString()), false);
}
...
void OwncloudSetupWizard::slotConnectToOCUrl( const QString& url )
{
    ...
    _ocWizard->appendToConfigurationLog(tr("Đang thử kết nối với %1 tại %2...")
...
void OwncloudSetupWizard::slotAuthError()
{
	...
	foreach(QByteArray head, headerList) {
		qDebug() << head << ":" << reply->rawHeader(head) << " Nghia \n";
	}
    ...
    if (!redirectUrl.isEmpty()) {
        ...
        errorMsg = tr("Yêu cầu xác thực tới máy chủ được chuyển hướng đến "
                      "'%1'. URL không đúng, máy chủ bị định cấu hình sai.")
	...
	if (reply->error() != QNetworkReply::NoError) 
	{
        if (!_ocWizard->account()->credentials()->stillValid(reply)) {
			...
			//Nghia add
			QString str = "";
			str = errorMessage(reply->errorString(), reply->readAll());
			QList<QByteArray> headerList = reply->rawHeaderList();
			foreach(QByteArray head, headerList) {
				str += reply->rawHeader(head);
			}
			if(str.contains("het_han_su_dung", Qt::CaseInsensitive))
			{
				errorMsg = tr("Tài khoản của bạn đã hết hạn sử dụng");
			}
			else
			{
				errorMsg = tr("Truy cập bị từ chối bởi máy chủ. Để xác minh rằng bạn có quyền truy cập thích hợp, "
                          "<a href=\"%1\">nhấn vào đây</a> để truy cập dịch vụ với trình duyệt của bạn.")
	...
	{
        errorMsg = tr("Đã có phản hồi không hợp lệ đối với yêu cầu webdav được xác thực");
    }
...
void OwncloudSetupWizard::slotCreateLocalAndRemoteFolders(const QString& localFolder, const QString& remoteFolder)
{
    ...
    if( fi.exists() ) {
        ...
        _ocWizard->appendToConfigurationLog( tr("Thư mục đồng bộ hoá trên máy tính %1 đã tồn tại, cài đặt nó cho đồng bộ.<br/><br/>").arg(localFolder));
    } else {
        QString res = tr("Đang tạo thư mục đồng bộ trên máy tính %1...").arg(localFolder);
        if( fi.mkpath( localFolder ) ) {
            ...
            _ocWizard->displayError(tr("Không thể tạo thư mục trên máy tính %1").arg(localFolder), false);
            nextStep = false;
        }
...
void OwncloudSetupWizard::slotRemoteFolderExists(QNetworkReply *reply)
{
    ...
    if( errId == QNetworkReply::NoError ) {
        ...
        if( _remoteFolder.isEmpty() ) {
            error = tr("Không có thư mục từ xa được chỉ định!");
            ok = false;
        } else {
            createRemoteFolder();
        }
    } else {
        error = tr("Lỗi: %1").arg(reply->errorString());
        ok = false;
    }
...
{
    _ocWizard->appendToConfigurationLog( tr("tạo thư mục trên VNPT DRIVE: %1" ).arg( _remoteFolder ));
...
void OwncloudSetupWizard::slotCreateRemoteFolderFinished( QNetworkReply::NetworkError error )
{
    ...
    if( error == QNetworkReply::NoError ) {
        _ocWizard->appendToConfigurationLog( tr("Thư mục %1 đã được tạo thành công.").arg(_remoteFolder));
    } else if( error == 202 ) {
        _ocWizard->appendToConfigurationLog( tr("Thư mục từ xa %1 đã tồn tại. Kết nối tới để đồng bộ").arg(_remoteFolder));
    } else if( error > 202 && error < 300 ) {
        _ocWizard->displayError( tr("Kết quả tạo thư mục trong HTTP error code %1").arg((int)error ), false);

        _ocWizard->appendToConfigurationLog( tr("Kết quả tạo thư mục trong HTTP error code %1").arg((int)error) );
    } else if( error == QNetworkReply::OperationCanceledError ) {
        _ocWizard->displayError( tr("Việc tạo thư mục không thành công vì các thông tin được cung cấp "
                                    "là sai!"
                                    "<br/>Làm ơn quay lại và kiểm tra thông tin của bạn.</p>"), false);
        _ocWizard->appendToConfigurationLog( tr("<p><font color=\"red\">Việc tạo thư mục không thành công vì các thông tin được cung cấp là sai.</font>"
                                                "<br/>Làm ơn quay lại và kiểm tra thông tin của bạn.</p>"));
        _remoteFolder.clear();
        success = false;
    } else {
        _ocWizard->appendToConfigurationLog( tr("Thư mục %1 tạo thất bại bởi lỗi <tt>%2</tt>.").arg(_remoteFolder).arg(error));
        _ocWizard->displayError( tr("Thư mục %1 tạo thất bại bởi lỗi <tt>%2</tt>.").arg(_remoteFolder).arg(error), false );
        _remoteFolder.clear();
        success = false;
    }
...
void OwncloudSetupWizard::finalizeSetup( bool success )
{
    ...
    if( success ) {
        if( !(localFolder.isEmpty() || _remoteFolder.isEmpty() )) {
            _ocWizard->appendToConfigurationLog( tr("Đồng bộ kết nối từ %1 tới đường dẫn %2 được thiết lập.")
        ...
        _ocWizard->appendToConfigurationLog(QLatin1String("<p><font color=\"red\">")
                                            + tr("Kết nối tới %1 không được thiết lập. Làm ơn kiểm tra lại")
                                            .arg(Theme::instance()->appNameGUI())
...
bool OwncloudSetupWizard::ensureStartFromScratch(const QString &localFolder) {
    ...
    while( !renameOk ) {
        ...
        if( ! renameOk ) {
            ...
            but = QMessageBox::question( 0, tr("Đổi tên thư mục thất bại"),
                                         tr("Không thể xóa hay đồng bộ thư mục vì thư mục hoặc tập tin đang được sử dụng ở chương trình khác."
                                            "Vui lòng đóng thư mục hoặc tập tin và nhấn thử lại hoặc hủy cài đặt."), QMessageBox::Retry | QMessageBox::Abort, QMessageBox::Retry);
