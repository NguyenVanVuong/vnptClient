    ...
    _ui(new Ui::NetworkSettings)
{
    ...
    _ui->userLineEdit->setPlaceholderText(tr("Tên đăng nhập proxy server"));
    _ui->passwordLineEdit->setPlaceholderText(tr("Mật khẩu proxy server"));
...
void NetworkSettings::loadBWLimitSettings()
{
    ...
    if (QLatin1String(v) < QLatin1String("5.3.3")) {
        QString tooltip = tr("Qt >= 5.4 là bắt buộc để sử dụng giới hạn băng thông");
