...
FolderStatusDelegate::FolderStatusDelegate() : QStyledItemDelegate() {
  //Create by Van Vuong- 3/1/2018 : Them cac bien luu icon
    m_openIcon = QIcon(QLatin1String(":/client/resources/open.png"));
    m_dellIcon = QIcon(QLatin1String(":/client/resources/dell.png"));
    m_playIcon = QIcon(QLatin1String(":/client/resources/play.png"));
    m_pauseIcon = QIcon(QLatin1String(":/client/resources/pause.png"));
	//close
}

QString FolderStatusDelegate::addFolderText()
{
    return tr("Thêm thư mục");
}
                                 ...
                                 const QModelIndex &index) const
{
    ...
    */
    ...
    bool folderPause      =qvariant_cast<bool>(index.data(FolderSyncPaused));
    ...
    //Create by Van Vuong- 3/1/2018 : 
    auto optionsButtonVisualRectOpen = optionsButtonRectOpen(option.rect, option.direction);
	auto optionsButtonVisualRectPause = optionsButtonRectPause(option.rect, option.direction);
	
	auto optionsButtonVisualRectDell = optionsButtonRectDell(option.rect, option.direction);
    ...
    if(!showProgess) {
        ...
        QString elidedRemotePathText = subFm.elidedText(
                tr("Đã đồng bộ với thư mục trên máy tính"),
    ...
    if (showProgess) {
        int space = 180;
        int fileNameTextHeight = subFm.boundingRect(tr("Tập tin")).height();
        int barHeight = 7; // same height as quota bar
        int overallWidth = option.rect.right() - aliasMargin - space - optionsButtonVisualRectDell.width() - nextToIcon;
    ...
    {
		//Create by Van Vuong - 3/1/2018 : Hien thi nut ra mna hinh


        QStyleOptionButton btnOpt1;
   
        btnOpt1.state = option.state;
        btnOpt1.state &= ~(QStyle::State_Selected | QStyle::State_HasFocus);
        btnOpt1.state |= QStyle::State_Raised;
       
        btnOpt1.rect =  optionsButtonVisualRectDell;
        btnOpt1.icon = m_dellIcon;

        btnOpt1.iconSize = btnOpt1.rect.size();
          QApplication::style()->drawControl(QStyle::CE_PushButton, &btnOpt1, painter);
    
    
        QStyleOptionButton btnOpt2;
     
        btnOpt2.state = option.state;
        btnOpt2.state &= ~(QStyle::State_Selected | QStyle::State_HasFocus);
        btnOpt2.state |= QStyle::State_Raised;
    
        btnOpt2.rect =  optionsButtonVisualRectPause;
        btnOpt2.icon = folderPause? m_playIcon : m_pauseIcon; 
     
        btnOpt2.iconSize = btnOpt2.rect.size();
         QApplication::style()->drawControl(QStyle::CE_PushButton, &btnOpt2, painter);
        
         QStyleOptionButton btnOpt3;
       
        btnOpt3.state = option.state;
        btnOpt3.state &= ~(QStyle::State_Selected | QStyle::State_HasFocus);
        btnOpt3.state |= QStyle::State_Raised;
      
        btnOpt3.rect =  optionsButtonVisualRectOpen;
        btnOpt3.icon = m_openIcon;
        btnOpt3.iconSize = btnOpt3.rect.size();
        QApplication::style()->drawControl(QStyle::CE_PushButton, &btnOpt3, painter);
		
		
	QString openStr = "Mở thư mục";
	QString pauseStr = folderPause? "Tiếp tục":"Tạm ngưng";
	QString dellStr = "Ngắt kết nối";
	QRect openText;
        openText.setTop(btnOpt3.rect.bottom() + 4 );
        openText.setHeight(14);
        openText.setLeft( btnOpt3.rect.left() - 1.5 );
        openText.setWidth(60);
		painter->drawText(QStyle::visualRect(option.direction, option.rect, openText),
                          Qt::AlignLeft | Qt::AlignVCenter, openStr);
		QRect pauseText; 
        pauseText.setTop(btnOpt2.rect.bottom() + 4 );
        pauseText.setHeight(14);
        pauseText.setLeft( btnOpt2.rect.left() );
        pauseText.setWidth(60);
		painter->drawText(QStyle::visualRect(option.direction, option.rect, pauseText),
                          Qt::AlignLeft | Qt::AlignVCenter, pauseStr);
		QRect dellText;
        dellText.setTop(btnOpt1.rect.bottom() + 4 );
        dellText.setHeight(14);
        dellText.setLeft( btnOpt1.rect.left() - 3 );
        dellText.setWidth(60);
		painter->drawText(QStyle::visualRect(option.direction, option.rect, dellText),
                          Qt::AlignLeft | Qt::AlignVCenter, dellStr);
		//Close edit
    }
}
...
//Create by Van Vuong - 3/1/2018 : Them cac ham dinh vi cho cac nut

QRect FolderStatusDelegate::optionsButtonRectDell(QRect within, Qt::LayoutDirection direction)
{
    QFont font = QFont();
    ...
    QRect r(QPoint(within.right() - size.width() - margin,
                within.top() + within.height() / 2 - size.height() / 2-10),
        size);
    return QStyle::visualRect(direction, within, r);
}
QRect FolderStatusDelegate::optionsButtonRectPause(QRect within, Qt::LayoutDirection direction)
{
    QFont font = QFont();
    QFont aliasFont = makeAliasFont(font);
    QFontMetrics fm(font);
   ...
   QSize size = QSize(50,25);
    int margin = QApplication::style()->pixelMetric(QStyle::PM_DefaultLayoutSpacing);
    QRect r(QPoint(within.right() - size.width() - margin-70,
                within.top() + within.height() / 2 - size.height() / 2-10),
        size);
    return QStyle::visualRect(direction, within, r);
}
QRect FolderStatusDelegate::optionsButtonRectOpen(QRect within, Qt::LayoutDirection direction)
{
    QFont font = QFont();
    QFont aliasFont = makeAliasFont(font);
    QFontMetrics fm(font);
    ...
    int margin = QApplication::style()->pixelMetric(QStyle::PM_DefaultLayoutSpacing);
    QRect r(QPoint(within.right() - size.width() - margin-140,
                within.top() + within.height() / 2 - size.height() / 2-10),size);
    return QStyle::visualRect(direction, within, r);
}
//Close
