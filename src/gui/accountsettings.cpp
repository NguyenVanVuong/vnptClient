    ...
    _quotaInfo(accountState)
{
    ...
    ui->connectLabel->setText(tr("Không có tài khoản được cấu hình."));
...
//Delete by Van Vuong : 4/1/2018 : khong dung toi
...
void AccountSettings::slotFolderListClicked(const QModelIndex& indx)
{
  /*
    ...
    */
	//Create by Van Vuong 3/1/2018 : Su ly khi click vao cac nut
    if (_model->classify(indx) == FolderStatusModel::RootFolder) {
        ...
        auto pos = tv->mapFromGlobal(QCursor::pos());
         ...
         if (FolderStatusDelegate::optionsButtonRectDell(tv->visualRect(indx), layoutDirection()).contains(pos)) {
        
			slotRemoveCurrentFolder();
           return ;
       }
      
        if (FolderStatusDelegate::optionsButtonRectPause(tv->visualRect(indx), layoutDirection()).contains(pos)) {
            
                    slotEnableCurrentFolder();
           return ;
       }
        if (FolderStatusDelegate::optionsButtonRectOpen(tv->visualRect(indx), layoutDirection()).contains(pos)) {
...
void AccountSettings::slotFolderWizardAccepted()
{
...

    {
        ...
        if (!dir.exists()) {
            ...
            if (!dir.mkpath(".")) {
                QMessageBox::warning(this, tr("Tạo thư mục không thành công"),
                                     tr("<p>Không thể tạo thư mục trên máy tính <i>%1</i>.")
    ...
    if( selected.isValid() ) {
        ...
        if( !alias.isEmpty() ) {
            ...
            QMessageBox messageBox(QMessageBox::Question,
                                   tr("Xác nhận xóa kết nối đồng bộ thư mục"),
                                   tr("<p>Bạn có thực sự muốn dừng đồng bộ thư mục <i>%1</i>?</p>"
				      "<p><b>Lưu ý:</b>Để đồng bộ vui lòng xóa tài khoản và đăng nhập lại !</p>"
                                      "<p>Việc này sẽ <b>không</b> xóa bất kỳ tập tin nào.</p>").arg(shortGuiLocalPath),
                                   QMessageBox::NoButton,
                                   this);
            QPushButton* yesButton =
                    messageBox.addButton(tr("Xóa kết nối đồng bộ thư mục"), QMessageBox::YesRole);
            messageBox.addButton(tr("Hủy"), QMessageBox::NoRole);
...
#endif
                QMessageBox msgbox(QMessageBox::Question, tr("Đang chạy đồng bộ"),
                                   tr("Hoạt động đồng bộ đang chạy. <br/> Bạn có muốn chấm dứt hoạt động không?"),
...
void AccountSettings::slotUpdateQuota(qint64 total, qint64 used)
{
    if( total > 0 ) {
        ...
        QString toolTip = tr("Đã sử dụng  %1 (%3%)/%2. Một vài thư mục, bao gồm các thư mục được chia sẻ trên mạng hoặc có thể có các giới hạn khác nhau.").arg(usedStr, totalStr, percentStr);
        ui->quotaInfoLabel->setText(tr("Đã sử dụng %1/%2").arg(usedStr, totalStr));
        ...
        if (total == 0 || total == -1) {
            ui->quotaInfoLabel->setText(tr("Hiện tại không có thông tin sử dụng bộ nhớ nào."));
        } else {
            QString usedStr = Utility::octetsToString(used);
            ui->quotaInfoLabel->setText(tr("%1 sử dụng").arg(usedStr));
        }
    }
}
...
void AccountSettings::slotAccountStateChanged(int state)
{
    if (_accountState) {
        ...
        if (AbstractCredentials *cred = account->credentials()) {
           serverWithUser = tr("%1 qua <i>%2</i>").arg(server, cred->user());
        }

        if (state == AccountState::Connected) {
            QStringList errors;
            if (account->serverVersionUnsupported()) {
                errors << tr("Phiên bản máy chủ %1 cũ và không được hỗ trợ! Hãy chấp nhận rủi ro.").arg(account->serverVersion());
            }
            showConnectionLabel( tr("Đã kết nối"), errors );
        } else if (state == AccountState::ServiceUnavailable) {
            showConnectionLabel( tr("Máy chủ %1 tạm thời không có.").arg(server) );
        } else if (state == AccountState::SignedOut) {
            showConnectionLabel( tr("Đăng xuất từ %1.").arg(serverWithUser) );
        } else {
            showConnectionLabel( tr("Không có kết nối với %1 tại %2.")
                                 .arg(Theme::instance()->appNameGUI(),
                                      server), _accountState->connectionErrors() );
        }
    } else {
        // ownCloud is not yet configured.
        showConnectionLabel( tr("Không %1 kết nối được cấu hình.").arg(Theme::instance()->appNameGUI()) );
    }
    ...
    if( _accountState ) {
        if( _accountState->isSignedOut() ) {
            _toggleSignInOutAction->setText(tr("Đăng nhập"));
        } else {
            _toggleSignInOutAction->setText(tr("Đăng xuất"));
        }
    }
}
...
void AccountSettings::refreshSelectiveSyncStatus()
{
    ...
    if (msg.isEmpty()) {
        ...
        QString wholeMsg = tr("Có những thư mục mới không được đồng bộ vì chúng quá lớn: ") + msg;
...
void AccountSettings::slotDeleteAccount()
{
    ...
    {
        QMessageBox messageBox(QMessageBox::Question,
                               tr("Xác nhận xóa tài khoản"),
                               tr("<p>Bạn có thật sự muốn xóa kết nối tới tài khoản <i>%1</i>?</p>"
			          "<p><b>Lưu ý:</b> Để đồng bộ vui lòng <b>xóa tài khoản và đăng nhập lại<b>!</p>"
                                  "<p>Điều này sẽ <b>không</b> xóa bất kỳ tập tin nào.</p>")
                                 .arg(_accountState->account()->displayName().replace(QString("."), QString(""))),
                               QMessageBox::NoButton,
                               this);
        QPushButton* yesButton =
                messageBox.addButton(tr("Xóa kết nối"), QMessageBox::YesRole);
        messageBox.addButton(tr("Hủy"), QMessageBox::NoRole);
