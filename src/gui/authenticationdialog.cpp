...
{
    setWindowTitle(tr("Yêu cầu xác thực"));
    QVBoxLayout *lay = new QVBoxLayout(this);
    QLabel *label = new QLabel(tr("Nhập tên người dùng và mật khẩu cho '%1' at %2.").arg(realm, domain));
    lay->addWidget(label);

    QFormLayout *form = new QFormLayout;
    form->addRow(tr("&Người dùng:"), _user);
    form->addRow(tr("&Mật khẩu:"), _password);
    lay->addLayout(form);
