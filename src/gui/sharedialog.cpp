    ...
    _progressIndicator(NULL)
{
    ...
    _ui->buttonBox->button(QDialogButtonBox::Close)->setText("Đóng");
    if( ocDir.isEmpty() ) {
        ...
        _ui->label_sharePath->setText(tr("Thư mục: %2").arg(ocDir));
    }

    this->setWindowTitle(tr("%1 Chia sẻ").arg(Theme::instance()->appNameGUI()));

    if (!accountState->account()->capabilities().shareAPI()) {
        _ui->shareWidgetsLayout->addWidget(new QLabel(tr("Máy chủ không cho phép chia sẻ")));
        return;
    }
    ...
    _progressIndicator->setToolTip(tr("Truy xuất quyền chia sẻ tối đa có thể từ máy chủ..."));
...
void ShareDialog::showSharingUi()
{
    ...
    if (!canReshare) {
        ...
        label->setText(tr("Không thể chia tệp này vì nó đã được chia sẻ mà không có sự cho phép chia sẻ."));
