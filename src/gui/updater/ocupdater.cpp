...
QString OCUpdater::statusString() const
{
    ...
    case Downloading:
        return tr("Đang tải xuống phiên bản %1. Vui lòng chờ...").arg(updateVersion);
    case DownloadComplete:
        return tr("%1 phiên bản %2 có hiệu lực. Khởi động lại ứng dụng để bắt đầu cập nhật.").arg(Theme::instance()->appNameGUI(), updateVersion);
    case DownloadFailed:
        return tr("Không thể tải xuống bản cập nhật. Vui lòng nhấn vào <a href='%1'>đây</a> để tải xuống thủ công.").arg(_updateInfo.web());
    case DownloadTimedOut:
        return tr("Không thể kiểm tra cập nhật mới.");
    case UpdateOnlyAvailableThroughSystem:
        return tr("Phiên bản %1 mới %2 có hiệu lực. Vui lòng sử dụng công cụ cập nhật hệ thống để cập nhật.").arg(Theme::instance()->appNameGUI(), updateVersion);
    case CheckingServer:
        return tr("Đã cập nhật bản mới nhất");//Đang kiểm tra cập nhật từ máy chủ...
    case Unknown:
        return tr("Trạng thái cập nhật chưa rõ: Không kiểm tra cập nhật mới.");
    case UpToDate:
        // fall through
    default:
        return tr("Không có sẵn bản cập nhật. Cài đặt của bạn ở phiên bản mới nhất.");
    }
}
...
void NSISUpdater::showDialog(const UpdateInfo &info)
{
    ...
    msgBox->setWindowTitle(tr("Có sẵn phiên bản mới"));

    QLabel *ico = new QLabel;
    ico->setFixedSize(iconSize, iconSize);
    ico->setPixmap(infoIcon.pixmap(iconSize));
    QLabel *lbl = new QLabel;
    QString txt = tr("<p>Một phiên bản mới của %1 có hiệu lực.</p>"
                     "<p><b>%2</b> có sẵn để tải xuống. Phiên bản đã cài là %3.</p>")
    ...
    QPushButton *skip = bb->addButton(tr("Bỏ qua phiên bản này"), QDialogButtonBox::ResetRole);
    QPushButton *reject = bb->addButton(tr("Bỏ qua thời gian này"), QDialogButtonBox::AcceptRole);
    QPushButton  *getupdate = bb->addButton(tr("Cập nhật"), QDialogButtonBox::AcceptRole);
