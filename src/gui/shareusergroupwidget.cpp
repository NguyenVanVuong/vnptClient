...
void ShareUserGroupWidget::slotShareesReady()
{
    ...
    if (_completerModel->rowCount() == 0) {
        displayError(0, tr("Không có kết quả cho '%1'").arg(_completerModel->currentSearch()));
        return;
    }
    _completer->complete();
}
