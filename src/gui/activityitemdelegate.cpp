                                 ...
                                 const QModelIndex &index) const
{
    ...
    // time rect
    QRect timeBox;
    int timeBoxWidth = fm.boundingRect(QLatin1String("4 giờ qua bởi longlongdomain.org")).width(); // FIXME.
    ...
    QString timeStr;
    if ( accountOnline ) {
        timeStr = tr("%1 trên %2").arg(timeText).arg(accountRole);
    } else {
        timeStr = tr("%1 trên %2 (ngắt kết nối)").arg(timeText).arg(accountRole);
