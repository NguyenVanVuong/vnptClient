...
void FolderStatusModel::setAccountStateV()
{
    beginResetModel();
    _dirty = false;
    _folders.clear();
...
QVariant FolderStatusModel::data(const QModelIndex &index, int role) const
{
    ...
    case AddButton: {
        if (role == FolderStatusDelegate::AddButton) {
            ...
            if (!_accountState->isConnected()) {
                return tr("Bạn cần phải được kết nối để thêm một thư mục");
            } if (_folders.count() == 1) {
                auto remotePath = _folders.at(0)._folder->remotePath();
                if (remotePath.isEmpty() || remotePath == QLatin1String("/")) {
                    // Syncing the entire owncloud: disable the add folder button (#3438)
                    return tr("Thêm thư mục bị vô hiệu vì bạn đã đồng bộ tất cả các tệp của mình. "
                            "Nếu bạn muốn đồng bộ nhiều thư mục, vui lòng xóa "
                            "cấu hình thư mục gốc.");
                }
            }
            return tr("Nhấp vào nút này để thêm một thư mục để đồng bộ.");
        }
        return QVariant();
    }
    case SubFolder:
    ...
    case FetchLabel:
    {
        ...
        switch(role) {
            case Qt::DisplayRole:
                if (x->_hasError) {
                    return tr("Lỗi khi tải danh sách thư mục từ máy chủ.");
                } else {
                    return tr("Tìm nạp danh sách thư mục từ máy chủ...");
                }
                break;
...
static QString shortenFilename( Folder *f, const QString& file )
{
    ...
    if(shortFile.startsWith(QLatin1String("vnptdrive://")) ||
            shortFile.startsWith(QLatin1String("vnptdrive://")) ) {
...
void FolderStatusModel::slotSetProgress(const ProgressInfo &progress)
{
    ...
    if (!progress._currentDiscoveredFolder.isEmpty()) {
        pi->_overallSyncString = tr("Đang kiểm tra thay đổi trong '%1'").arg(progress._currentDiscoveredFolder);
    ...
    if (ProgressInfo::isSizeDependent(curItem)) {
        ...
        if (estimatedUpBw || estimatedDownBw) {
            ...
            */
            ...
            fileProgressString = tr("Đang đồng bộ %1").arg(allFilenames);
...
// ifdefs: https://github.com/owncloud/client/issues/3095#issuecomment-128409294
                ...
                fileProgressString.append(tr("tải xuống %1/s").arg(Utility::octetsToString(estimatedDownBw)));
#else
            ...
            if (estimatedUpBw > 0) {
                ...
                fileProgressString.append(tr("tải lên %1/s").arg(Utility::octetsToString(estimatedUpBw)));
#else
            ...
            fileProgressString = tr("%1 %2 (%3 của %4)") .arg(kindString, itemFileName, s1, s2);
        }
    ...
    if (totalSize > 0) {
        ...
        QString s3 = Utility::durationToDescriptiveString1(progress.totalProgress().estimatedEta);
		...
		else if(s3.contains("hour"))
		{
			s3.replace("hour","giờ");
			
		}
		else if(s3.contains("minute"))
		{
			s3.replace("minute","phút");
		}
		else if(s3.contains("second"))
		{
			s3.replace("second","giây");
		}
		else if(s3.contains("month"))
		{
			s3.replace("month","tháng");
		}
        //: Example text: "5 minutes left, 12 MB of 345 MB, file 6 of 7"
        overallSyncString = tr("%5 còn lại, %1 của %2, tệp tin %3 của %4")
            .arg(s1, s2)
            .arg(currentFile).arg(totalFileCount)
            .arg(s3) ;
    } else if (totalFileCount > 0) {
        // Don't attempt to estimate the time left if there is no kb to transfer.
        overallSyncString = tr("tệp tin %1 của %2") .arg(currentFile).arg(totalFileCount);
    }
...
void FolderStatusModel::slotFolderSyncStateChange(Folder *f)
{
    ...
    if (f->syncPaused()) {
        ...
        if (pos <= 0) {
            ...
            message = tr("Đang chờ cho %n thư mục khác(s)...", "", pos);
        }
        _folders[folderIndex]._progress = SubFolderInfo::Progress();
        _folders[folderIndex]._progress._overallSyncString = message;
    } else if (state == SyncResult::SyncPrepare) {
        _folders[folderIndex]._progress = SubFolderInfo::Progress();
        _folders[folderIndex]._progress._overallSyncString = tr("Đang chuẩn bị đồng bộ hóa...");
