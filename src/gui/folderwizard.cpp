...
QString FormatWarningsWizardPage::formatWarnings(const QStringList &warnings) const
{
    QString ret;
    if (warnings.count() == 1) {
        ret = tr("<b>Cảnh báo:</b> %1").arg(warnings.first());
    } else if (warnings.count() > 1) {
        ret = tr("<b>Cảnh báo:</b>") + " <ul>";
    ...
    : FormatWarningsWizardPage()
{
    ...
    _ui.localFolderChooseBtn->setToolTip(tr("Nhấp để chọn một thư mục trên máy tính để đồng bộ."));

    QString defaultPath = QString::fromLatin1( "%1/%2").arg( QDir::homePath() ).arg(Theme::instance()->appName() );
    _ui.localFolderLineEdit->setText( QDir::toNativeSeparators( defaultPath ) );
    _ui.localFolderLineEdit->setToolTip(tr("Nhập đường dẫn đến thư mục trên máy tính."));
...
void FolderWizardRemotePath::slotAddRemoteFolder()
{
    ...
    dlg->setWindowTitle(tr("Tạo thư mục từ xa"));
    dlg->setLabelText(tr("Nhập tên của thư mục mới được tạo bên dưới '%1':")
                      .arg(parent));
...
void FolderWizardRemotePath::slotCreateRemoteFolderFinished(QNetworkReply::NetworkError error)
{
    if (error == QNetworkReply::NoError) {
        ...
        showWarn(tr("Đã tạo thành công thư mục trên %1.").arg(Theme::instance()->appNameGUI()));
        slotRefreshFolders();
...
void FolderWizardRemotePath::slotHandleMkdirNetworkError(QNetworkReply *reply)
{
    ...
    if( reply && !_account->credentials()->stillValid(reply) ) {
        showWarn(tr("Xác thực không truy cập được %1").arg(Theme::instance()->appNameGUI()));
    } else {
        showWarn(tr("Không thể tạo thư mục trên %1. Vui lòng kiểm tra thủ công.")
...
void FolderWizardRemotePath::slotUpdateDirectories(const QStringList &list)
{
    ...
    if (!root) {
        ...
        root->setToolTip(0, tr("Chọn mục này để đồng bộ toàn bộ tài khoản"));
...
bool FolderWizardRemotePath::isComplete() const
{
    ...
    for(i = map.constBegin();i != map.constEnd(); i++ ) {
        ...
        if (QDir::cleanPath(dir) == QDir::cleanPath(curDir)) {
            warnStrings.append(tr("Thư mục này đã được đồng bộ."));
        } else if (dir.startsWith(curDir + QLatin1Char('/'))) {
            warnStrings.append(tr("Bạn đã đồng bộ <i>%1</i>, đó là thư mục cha của <i>%2</i>.").arg(curDir).arg(dir));
        }

        if (curDir == QLatin1String("/")) {
            warnStrings.append(tr("Bạn đã đồng bộ tất cả các tập tin của mình. đồng bộ thư mục khác <b>không</ b> được hỗ trợ. "
                                  "Nếu bạn muốn đồng bộ nhiều thư mục, vui lòng xóa cấu hình hiện tại "
                                  "đồng bộ thư mục gốc."));
        }
    }
...
FolderWizardSelectiveSync::FolderWizardSelectiveSync(AccountPtr account)
{
    ...
    layout->addWidget(new QLabel(tr("Chọn Nội dung cần đồng bộ: Bạn có thể bỏ chọn các thư mục con từ xa mà bạn không muốn đồng bộ.")));
    ...
    _folderWizardSelectiveSyncPage(new FolderWizardSelectiveSync(account))
{
    ...
    setWindowTitle( tr("Thêm thư mục cần đồng bộ") );
    setOptions(QWizard::CancelButtonOnLeft);
    setButtonText(QWizard::FinishButton, tr("Thêm thư mục"));
}
