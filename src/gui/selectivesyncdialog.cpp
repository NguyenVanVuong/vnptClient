...
{
    _loading = new QLabel(tr("Đang tải ..."), this);
    ...
    headerItem()->setText(0, tr("Tên"));
    headerItem()->setText(1, tr("Kích thước"));
}
...
void SelectiveSyncTreeView::slotUpdateDirectories(QStringList list)
{
    ...
    if (!root && list.size() <= 1) {
        _loading->setText(tr("Không có thư mục con hiện tại trên máy chủ."));
...
void SelectiveSyncTreeView::slotLscolFinishedWithError(QNetworkReply *r)
{
    if (r->error() == QNetworkReply::ContentNotFoundError) {
        ...
        _loading->setText(tr("Đã xảy ra lỗi khi tải danh sách các thư mục con."));
    }
    _loading->resize(_loading->sizeHint()); // because it's not in a layout
}
...
{
    bool ok;
    init(account, tr("Các thư mục không được kiểm tra sẽ được<b>xóa</b> từ hệ thống tệp cục bộ của bạn và sẽ không được đồng bộ hóa với máy tính này nữa"));
    ...
    : QDialog(parent, f), _folder(0)
{
    init(account,
         Theme::instance()->wizardSelectiveSyncDefaultNothing() ?
            tr("Chọn nội dung cần đồng bộ hóa: Chọn các thư mục con bạn muốn đồng bộ hóa.") :
            tr("Chọn nội dung để đồng bộ hóa: Bỏ chọn các thư mục con bạn không muốn đồng bộ hóa."));
...
{
    setWindowTitle(tr("Chọn nội dung để đồng bộ"));
