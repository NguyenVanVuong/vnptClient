    ...
    _passwordRequired(false)
{
    ...
    _ui->checkBox_password->setText(tr("Mật khẩu bảo vệ"));
...
void ShareLinkWidget::slotPasswordSet()
{
    ...
    _ui->lineEdit_password->setPlaceholderText(tr("Mật khẩu bảo vệ"));
...
void ShareLinkWidget::slotSharesFetched(const QList<QSharedPointer<Share>> &shares)
{
    ...
    if( !_share.isNull() ) {
        ...
        if( !(_maxSharingPermissions & SharePermissionShare) ) {
            displayError(tr("Không thể chia sẻ tệp vì nó đã được chia sẻ mà không có sự cho phép chia sẻ."));
            _ui->checkBox_shareLink->setEnabled(false);
...
void ShareLinkWidget::slotCheckBoxShareLinkClicked()
{
    ...
    if (_ui->checkBox_shareLink->checkState() == Qt::Checked) {
        ...
        if (_account->capabilities().sharePublicLinkEnforcePassword()) {
            _ui->checkBox_password->setChecked(true);
            _ui->checkBox_password->setEnabled(false);
            _ui->checkBox_password->setText(tr("Công khai và yêu cầu mật khẩu"));
            _ui->checkBox_expire->setEnabled(false);
...
void ShareLinkWidget::slotCheckBoxPasswordClicked()
{
    if (_ui->checkBox_password->checkState() == Qt::Checked) {
        _ui->lineEdit_password->show();
        _ui->pushButton_setPassword->show();
        _ui->lineEdit_password->setPlaceholderText(tr("Vui lòng thiết lập mật khẩu"));
        _ui->lineEdit_password->setFocus();
    } else {
        setPassword(QString());
...
{
    const QString noSharesTitle(tr("&Chia sẻ liên kết"));
    const QString haveSharesTitle(tr("&Chia sẻ liên kết"));
