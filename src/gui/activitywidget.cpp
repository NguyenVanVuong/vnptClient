    ...
    _notificationRequestsRunning(0)
{
    ...
    _copyBtn = _ui->_dialogButtonBox->addButton("Sao chép", QDialogButtonBox::ActionRole);
    _copyBtn->setToolTip("Sao chép các hoạt động vào bộ nhớ");
...
{
    QString t = "Các hoạt động";
    _ui->_headerLabel->setTextFormat(Qt::RichText);
    _ui->_headerLabel->setText(t);

    _ui->_notifyLabel->setText("Hành động Bắt buộc: Thông báo");

    t.clear();
    QSetIterator<QString> i(_accountsWithoutActivities);
    while (i.hasNext() ) {
        t.append( tr("<br/>Tài khoản %1 không có hoạt động nào được kích hoạt.").arg(i.next()));
    }
...
void ActivityWidget::slotBuildNotificationDisplay(const ActivityList& list)
{
    ...
    if( newGuiLogCount > 0 ) {
        ...
        QString msg = tr("Bạn đã nhận %n thông báo mới(s) từ %2.", "", accNotified[accNotified.keys().at(0)]).
        ...
        if( newGuiLogCount >= 2 ) {
            ...
            if( newGuiLogCount == 2 ) {
                ...
                msg = tr("Bạn đã nhận %n thông báo mới(s) từ %1 và %2.", "", notiCount).arg(acc1, acc2);
            } else {
                msg = tr("Bạn đã nhận thông báo mới từ %1, %2 và những tài khoản khác.").arg(acc1, acc2);
            }
        }

        const QString log = tr("%1 Thông báo - Cần hành động").arg(Theme::instance()->appNameGUI());
        emit guiLog( log, msg);
    }
    ...
    :QWidget(parent)
{
    ...
    _activityTabId = _tab->insertTab(0, _activityWidget, Theme::instance()->applicationIcon(), "Hoạt động của người dùng");
    ...
    _tab->insertTab(1, _protocolWidget, Theme::instance()->syncStateIcon(SyncResult::Success), "Giao thức đồng bộ");
    ...
    vbox2->addWidget(new QLabel("Danh sách tệp bị bỏ qua hoặc có lỗi", this));
    ...
    QPushButton *_copyBtn = dlgButtonBox->addButton(tr("Sao chép"), QDialogButtonBox::ActionRole);
    _copyBtn->setToolTip("Sao chép danh sách hoạt động vào bộ nhớ tạm.");
...
{
    QString cntText = "Không đồng bộ được";
    if( cnt ) {
        //: %1 is the number of not synced files.
        cntText = tr("Không đồng bộ được (%1)").arg(cnt);
    }
...
void ActivitySettings::slotCopyToClipboard()
{
    ...
    if( idx == 0 ) {
        ...
        message = "Danh sách hoạt động đã được sao chép vào bộ nhớ tạm.";
    } else if(idx == 1 ) {
        // the protocol widget
        _protocolWidget->storeSyncActivity(ts);
        message = "Danh sách hoạt động đồng bộ đã được sao chép vào bộ nhớ tạm.";
    } else if(idx == 2 ) {
        // issues Widget
        message = "Danh sách các mục chưa đồng bộ đã được sao chép vào bộ nhớ tạm.";
