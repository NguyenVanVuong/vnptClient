...
bool FolderMan::ensureJournalGone(const QString &localPath)
{
    ...
    while (QFile::exists(stateDbFile) && !QFile::remove(stateDbFile)) {
        ...
        int ret = QMessageBox::warning(0, tr("Không thể đặt lại trạng thái thư mục"),
                                       tr("Một thông báo đồng bộ cũ '%1' đã tìm thấy, "
                                          "nhưng không thể xóa. Làm ơn chắc chắn "
                                          "rằng không có ứng dụng nào đang sử dụng nó.")
...
Folder* FolderMan::setupFolderFromOldConfigFile(const QString &file, AccountState *accountState )
{
    ...
    if (backend.isEmpty() || backend != QLatin1String("vntpcloud")) {
...
QString FolderMan::statusToString( SyncResult syncStatus, bool paused ) const
{
    ...
    case SyncResult::Undefined:
        folderMessage = tr( "Trạng thái chưa xác định." );
        break;
    case SyncResult::NotYetStarted:
        folderMessage = tr( "Đang đợi để bắt đầu đồng bộ." );
        break;
    case SyncResult::SyncPrepare:
        folderMessage = tr( "Đang chuẩn bị đồng bộ." );
        break;
    case SyncResult::SyncRunning:
        folderMessage = tr( "Đồng bộ đang chạy." );
        break;
    case SyncResult::Success:
        folderMessage = tr( "Đồng bộ lần cuối đã thành công." );
        break;
    case SyncResult::Error:
        break;
    case SyncResult::Problem:
        folderMessage = tr( "Lần đồng bộ cuối cùng đã thành công, nhưng có các cảnh báo trên các tạp tin tin cá nhân.");
        break;
    case SyncResult::SetupError:
        folderMessage = tr( "Lỗi cài đặt." );
        break;
    case SyncResult::SyncAbortRequested:
        folderMessage = tr( "User Abort." );
        break;
    case SyncResult::Paused:
        folderMessage = tr("Đồng bộ bị ngừng.");
        break;
    // no default case on purpose, check compiler warnings
    }
    if( paused ) {
        // sync is disabled.
        folderMessage = tr( "%1 (Đồng bộ bị ngừng)" ).arg(folderMessage);
    }
...
{
    if (path.isEmpty()) {
        return tr("Không có thư mục hợp lệ nào được chọn!");
    }
    ...
    if( !selFile.isDir() ) {
        return tr("Đường dẫn đã chọn không phải là thư mục!");
    }

    if ( !selFile.isWritable() ) {
        return tr("Bạn không có quyền ghi vào thư mục đã chọn!");
    }
    ...
    for (auto i = _folderMap.constBegin(); i != _folderMap.constEnd(); ++i ) {
        ...
        if (QDir::cleanPath(f->path()) == QDir::cleanPath(userInput)
            ...
            return tr("Thư mục trên máy tính %1 đã được sử dụng trong một kết nối đồng bộ thư mục. "
                      "Vui lòng chọn một cái khác!")
                .arg(QDir::toNativeSeparators(userInput));
        }
        if (!forNewDirectory && QDir::cleanPath(folderDir).startsWith(QDir::cleanPath(userInput)+'/')) {
            return tr("Thư mục trên máy tính %1 đã chứa một thư mục được sử dụng trong một kết nối đồng bộ thư mục. "
                      "Vui lòng chọn một cái khác!")
                .arg(QDir::toNativeSeparators(userInput));
        }
        ...
        if (!forNewDirectory && QDir::cleanPath(folderDir).startsWith(absCleanUserFolder) ) {
            return tr("Thư mục trên máy tính %1 là một liên kết tượng trưng. "
                      "Mục tiêu liên kết đã chứa một thư mục được sử dụng trong một kết nối đồng bộ thư mục. "
                      "Vui lòng chọn một cái khác!")
                .arg(QDir::toNativeSeparators(userInput));
        }

        if (QDir::cleanPath(QString(userInput)).startsWith( QDir::cleanPath(folderDir)+'/')) {
            return tr("Thư mục trên máy tính %1 đã được chứa trong một thư mục được sử dụng trong một kết nối đồng bộ thư mục. "
                      "Vui lòng chọn một cái khác!")
                .arg(QDir::toNativeSeparators(userInput));
        }
        ...
        if (absCleanUserFolder.startsWith( QDir::cleanPath(folderDir)+'/')) {
            return tr("Thư mục trên máy tính %1 là một liên kết tượng trưng. "
                      "Mục tiêu liên kết đã có trong thư mục được sử dụng trong một kết nối đồng bộ thư mục."
                      "Vui lòng chọn một cái khác!")
                .arg(QDir::toNativeSeparators(userInput));
        }
    }
