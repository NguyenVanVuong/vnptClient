...
void ownCloudGui::slotComputeOverallSyncStatus()
{
    ...
    if (!problemAccounts.empty()) {
        ...
        _tray->setToolTip(tr("Ngắt kết nối từ %1").arg(
                accountNames.join(QLatin1String(", "))));
#else
        QStringList messages;
        messages.append(tr("Ngắt kết nối từ tài khoản:"));
        foreach (AccountStatePtr a, problemAccounts) {
            QString message = tr("Tài khoản %1: %2").arg(
    ...
    if (allSignedOut) {
        _tray->setIcon(Theme::instance()->folderOfflineIcon(true));
        _tray->setToolTip(tr("Vui lòng đăng nhập"));
        return;
    } else if (allPaused) {
        _tray->setIcon(Theme::instance()->syncStateIcon(SyncResult::Paused, true));
        _tray->setToolTip(tr("Tài khoản đồng bộ không được kích hoạt"));
        return;
    }
    ...
    if( overallResult.status() != SyncResult::Undefined ) {
        if( map.count() > 0 ) {
            ...
            foreach(Folder* folder, map.values()) {
                ...
                allStatusStrings += tr("Thư mục %1: %2").arg(folder->shortGuiLocalPath(), folderMessage);
            }
            trayMessage = allStatusStrings.join(QLatin1String("\n"));
#endif
        } else {
            trayMessage = tr("Không có thư mục đồng bộ nào được cấu hình.");
        }
...
void ownCloudGui::addAccountContextMenu(AccountStatePtr accountState, QMenu *menu, bool separateMenu)
{
    ...
    // account sub menu.
    QString browserOpen = tr("Mở với trình duyệt");
    if (!separateMenu) {
        browserOpen = tr("Mở %1 trong trình duyệt").arg(Theme::instance()->appNameGUI());
    }
    ...
    foreach (Folder* folder, folderMan->map()) {
        ...
        if (firstFolder && !singleSyncFolder) {
            firstFolder = false;
            menu->addSeparator();
            menu->addAction(tr("Quản lý thư mục:"))->setDisabled(true);
        }

        QAction *action = new QAction( tr("Mở thư mục '%1'").arg(folder->shortGuiLocalPath()), menu );
     ...
     if (separateMenu) {
         if (onePaused) {
             QAction* enable = menu->addAction(tr("Bỏ tạm dừng tất cả các thư mục"));
...
void ownCloudGui::setupContextMenu()
{
    ...
    if (minimalTrayMenu()) {
        if (!_contextMenu) {
            ...
            _recentActionsMenu = new QMenu(tr("Những thay đổi gần đây"), _contextMenu.data());
    ...
    if (atLeastOnePaused) {
        QString text;
        if (accountList.count() > 1) {
            text = tr("Ngừng tất cả đồng bộ");
        } else {
            text = tr("Bỏ tạm ngừng đồng bộ");
        }
    ...
    if (atLeastOneSignedIn) {
        if (accountList.count() > 1) {
            _actionLogout->setText(tr("Đăng xuất khỏi tất cả các tài khoản"));
        } else {
            _actionLogout->setText(tr("Đăng xuất"));
        }
        _contextMenu->addAction(_actionLogout);
    }
    if (atLeastOneSignedOut) {
        if (accountList.count() > 1) {
            _actionLogin->setText(tr("Đăng nhập tất cả tài khoản..."));
        } else {
            _actionLogin->setText(tr("Đăng nhập..."));
        }
...
{
    _actionStatus = new QAction(tr("Không biết trạng thái"), this);
    _actionStatus->setEnabled( false );
    _actionSettings = new QAction(tr("Thiết lập..."), this);
    _actionRecent = new QAction(tr("Chi tiết..."), this);
    ...
    _actionHelp = new QAction(tr("Giúp đỡ"), this);
    QObject::connect(_actionHelp, SIGNAL(triggered(bool)), SLOT(slotHelp()));
    _actionQuit = new QAction(tr("Thoát %1").arg(Theme::instance()->appNameGUI()), this);
    QObject::connect(_actionQuit, SIGNAL(triggered(bool)), _app, SLOT(quit()));

    _actionLogin = new QAction(tr("Đăng nhập..."), this);
    connect(_actionLogin, SIGNAL(triggered()), this, SLOT(slotLogin()));
    _actionLogout = new QAction(tr("Đăng xuất"), this);
    connect(_actionLogout, SIGNAL(triggered()), this, SLOT(slotLogout()));

    if(_app->debugMode()) {
        _actionCrash = new QAction(tr("Crash ngay bây giờ", "Chỉ hiển thị trong chế độ gỡ lỗi để cho phép kiểm tra trình xử lý sự cố"), this);
...
void ownCloudGui::slotRebuildRecentMenus()
{
    ...
    if (!_recentItemsActions.isEmpty()) {
        ...
        _recentActionsMenu->addAction(tr("Không có mục nào được đồng bộ gần đây"))->setEnabled(false);
    }
    // add a more... entry.
...
void ownCloudGui::slotUpdateProgress(const QString &folder, const ProgressInfo& progress)
{
    ...
    if (!progress._currentDiscoveredFolder.isEmpty()) {
        _actionStatus->setText( tr("Kiểm tra các thay đổi trong '%1'")
                                .arg( progress._currentDiscoveredFolder ));
    } else if (progress.totalSize() == 0 ) {
        quint64 currentFile = progress.currentFile();
        quint64 totalFileCount = qMax(progress.totalFiles(), currentFile);
        _actionStatus->setText( tr("Đang đồng bộ %1 of %2  (%3 left)")
            .arg( currentFile ).arg( totalFileCount )
            .arg( Utility::durationToDescriptiveString2(progress.totalProgress().estimatedEta) ) );
    } else {
        QString totalSizeStr = Utility::octetsToString( progress.totalSize() );
        _actionStatus->setText( tr("Đang đồng bộ %1 (%2 left)")
...
{
    _actionStatus->setText(tr("Cập nhật"));
}
