...
void ValidateChecksumHeader::start(const QString& filePath, const QByteArray& checksumHeader)
{
    ...
    if( !parseChecksumHeader(checksumHeader, &_expectedChecksumType, &_expectedChecksum) ) {
        ...
        emit validationFailed(tr("Tiêu đề kiểm tra bị sai."));
        return;
    }
                                                    ...
                                                    const QByteArray& checksum)
{
    if( checksumType != _expectedChecksumType ) {
        emit validationFailed(tr("Tiêu đề kiểm tra có loại kiểm tra không xác định '%1'").arg(
                                  QString::fromLatin1(_expectedChecksumType)));
        return;
    }
    if( checksum != _expectedChecksum ) {
        emit validationFailed(tr("Tệp đã tải xuống không khớp với tổng kiểm tra, nó sẽ được tiếp tục."));
        return;
    }
