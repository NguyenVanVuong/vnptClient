...
void Logger::setLogFile(const QString & name)
{
    ...
    if(!openSucceeded) {
        ...
        postGuiMessage( tr("Lỗi"),
                        QString(tr("<nobr>Thư mục '%1'<br/>không được mở để ghi.<br/><br/>"
                                   "Giá trị log xuất ra <b>không</b> được lưu lại!</nobr>"))
                        .arg(name));
        return;
    }
...
void Logger::enterNextLogFile()
{
    if (!_logDirectory.isEmpty()) {
        ...
        QStringList files = dir.entryList(QStringList("vnptdrive.log.*"),
                                    QDir::Files);
        QRegExp rx("vnptdrive.log.(\\d+)");
        uint maxNumber = 0;
        ...
        QString filename = _logDirectory + "/vnptdrive.log." + QString::number(maxNumber+1);
        setLogFile(filename);
    }
}
