...
QString SyncEngine::csyncErrorToString(CSYNC_STATUS err)
{
    ...
    case CSYNC_STATUS_OK:
        errStr = tr("Thành công.");
        break;
    case CSYNC_STATUS_STATEDB_LOAD_ERROR:
        errStr = tr("CSync không thể tải hoặc tạo tệp nhật ký. "
                    "Đảm bảo rằng bạn đã đọc và ghi quyền trong thư mục đồng bộ trên máy tính.");
        break;
    case CSYNC_STATUS_STATEDB_CORRUPTED:
        errStr = tr("CSync không thể tải tệp nhật ký. Tập tin nhật ký bị hỏng.");
        break;
    case CSYNC_STATUS_NO_MODULE:
        errStr = tr("<p>%1 không thể tải plugin cho csync. <br/> Vui lòng xác minh cài đặt! </ p>").arg(qApp->applicationName());
        break;
    case CSYNC_STATUS_TREE_ERROR:
        errStr = tr("CSync đã gặp lỗi trong khi xử lý cây nội bộ.");
        break;
    case CSYNC_STATUS_MEMORY_ERROR:
        errStr = tr("CSync không lưu trữ bộ nhớ.");
        break;
    case CSYNC_STATUS_PARAM_ERROR:
        errStr = tr("CSync lỗi tham số.");
        break;
    case CSYNC_STATUS_UPDATE_ERROR:
        errStr = tr("CSync cập nhật bước xử lý không thành công.");
        break;
    case CSYNC_STATUS_RECONCILE_ERROR:
        errStr = tr("CSync bước xử lý phục hồi không thành công.");
        break;
    case CSYNC_STATUS_PROXY_AUTH_ERROR:
        errStr = tr("CSync không thể xác thực tại proxy.");
        break;
    case CSYNC_STATUS_LOOKUP_ERROR:
        errStr = tr("CSync không tìm kiếm proxy hoặc máy chủ.");
        break;
    case CSYNC_STATUS_SERVER_AUTH_ERROR:
        errStr = tr("CSync không xác thực được tại máy chủ %1.").arg(qApp->applicationName());
        break;
    case CSYNC_STATUS_CONNECT_ERROR:
        errStr = tr("CSync không thể kết nối với mạng.");
        break;
    case CSYNC_STATUS_TIMEOUT:
        errStr = tr("Đã hết thời gian chờ kết nối mạng.");
        break;
    case CSYNC_STATUS_HTTP_ERROR:
        errStr = tr("Đã xảy ra lỗi truyền HTTP.");
        break;
    case CSYNC_STATUS_PERMISSION_DENIED:
        errStr = tr("CSync thất bại vì quyền truy cập bị từ chối.");
        break;
    case CSYNC_STATUS_NOT_FOUND:
        errStr = tr("CSync truy cập thất bại") + " "; // filename gets added.
        break;
    case CSYNC_STATUS_FILE_EXISTS:
        errStr = tr("CSync cố gắng tạo thư mục đã tồn tại.");
        break;
    case CSYNC_STATUS_OUT_OF_SPACE:
        errStr = tr("CSync: Lưu trữ trên máy chủ %1 khả dụng.").arg(qApp->applicationName());
        break;
    case CSYNC_STATUS_UNSUCCESSFUL:
        errStr = tr("CSync lỗi không xác định.");
        break;
    case CSYNC_STATUS_ABORTED:
        errStr = tr("Bị hủy bởi người sử dụng");
        break;
    case CSYNC_STATUS_SERVICE_UNAVAILABLE:
        errStr = tr("Dịch vụ tạm thời không khả dụng");
        break;
    case CSYNC_STATUS_STORAGE_UNAVAILABLE:
        errStr = tr("Thư mục đã được cài đặt tạm thời không khả dụng trên máy chủ");
        break;
    case CSYNC_STATUS_FORBIDDEN:
        errStr = tr("Truy cập bị từ chối");
        break;
    case CSYNC_STATUS_OPENDIR_ERROR:
        errStr = tr("Đã xảy ra lỗi khi mở thư mục");
        break;
    case CSYNC_STATUS_READDIR_ERROR:
        errStr = tr("Lỗi khi đọc thư mục.");
        break;
    case CSYNC_STATUS_INVALID_CHARACTERS:
        // Handled in callee
    default:
        errStr = tr("Đã xảy ra lỗi nội bộ số %1.").arg( (int) err );
    }
...
bool SyncEngine::checkErrorBlacklisting( SyncFileItem &item )
{
    ...
    item._errorString = tr("Thư mục không được đồng bộ: %1").arg(entry._errorString);
...
int SyncEngine::treewalkFile( TREE_WALK_FILE *file, bool remote )
{
    ...
    case CSYNC_STATUS_INDIVIDUAL_IS_SYMLINK:
        item->_errorString = tr("Các liên kết tượng trưng không được hỗ trợ trong quá trình đồng bộ.");
        break;
    case CSYNC_STATUS_INDIVIDUAL_IGNORE_LIST:
        item->_errorString = tr("Tập tin được liệt kê trong danh sách bỏ qua.");
        break;
    case CSYNC_STATUS_INDIVIDUAL_IS_INVALID_CHARS:
        item->_errorString = tr("Tên tập tin chứa các ký tự không hợp lệ, không thể đồng bộ.");
        break;
    case CSYNC_STATUS_INDIVIDUAL_TRAILING_SPACE:
        item->_errorString = tr("Tên tập tin có chứa dấu cách.");
        break;
    case CSYNC_STATUS_INDIVIDUAL_EXCLUDE_LONG_FILENAME:
        item->_errorString = tr("Tên tập tin quá dài.");
        break;
    case CSYNC_STATUS_INDIVIDUAL_EXCLUDE_HIDDEN:
        item->_errorString = tr("Tập tin/Thư mục bị bỏ qua vì tập tin này bị ẩn.");
        break;
    case CYSNC_STATUS_FILE_LOCKED_OR_OPEN:
        item->_errorString = QLatin1String("File locked"); // don't translate, internal use!
        break;
    case CSYNC_STATUS_INDIVIDUAL_STAT_FAILED:
        item->_errorString = tr("Thống kê không thành công.");
        break;
    case CSYNC_STATUS_SERVICE_UNAVAILABLE:
        item->_errorString = QLatin1String("Máy chủ tạm thời không hoạt động.");
        break;
    case CSYNC_STATUS_STORAGE_UNAVAILABLE:
        item->_errorString = QLatin1String("Thư mục tạm thời không khả dụng trên máy chủ.");
        item->_status = SyncFileItem::SoftError;
        _temporarilyUnavailablePaths.insert(item->_file);
        break;
    case CSYNC_STATUS_FORBIDDEN:
        item->_errorString = QLatin1String("Truy cập bị cấm.");
        item->_status = SyncFileItem::SoftError;
        _temporarilyUnavailablePaths.insert(item->_file);
        break;
    case CSYNC_STATUS_PERMISSION_DENIED:
        item->_errorString = QLatin1String("Thư mục không được truy cập từ máy trạm,Từ chối truy cập từ máy trạm.");
        item->_status = SyncFileItem::SoftError;
        break;
    default:
    ...
    if (item->_instruction == CSYNC_INSTRUCTION_IGNORE && (utf8State.invalidChars > 0 || utf8State.remainingChars > 0)) {
        ...
        //item->_instruction = CSYNC_INSTRUCTION_ERROR;
        item->_errorString = tr("Mã hóa tên tệp không hợp lệ");
    }
...
void SyncEngine::handleSyncError(CSYNC *ctx, const char *state) {
    ...
    if (err == CSYNC_STATUS_INVALID_CHARACTERS) {
        errStr = tr("Ký tự không hợp lệ, xin vui lòng đổi tên \"%1\"").arg(errMsg);
    }

    // if there is csyncs url modifier in the error message, replace it.
    if( errStr.contains("vnptdrives://") ) errStr.replace("vnptdrives://", "https://");
    if( errStr.contains("vnptdrive://") ) errStr.replace("vnptdrive://", "http://");
...
void SyncEngine::startSync()
{
    ...
    if (freeBytes >= 0) {
        ...
        if (freeBytes < minFree) {
            emit csyncError(tr("Chỉ có %1 có sẵn, cần ít nhất %2 để bắt đầu",
                               "Placeholders được postfixed với kích thước tập tin bằng cách sử dụng Utility::octetsToString()").arg(
    ...
    if( fileRecordCount == -1 ) {
        ...
        emit csyncError(tr("Không thể khởi tạo một nhật ký đồng bộ."));
        finalize(false);
        return;
    ...
    if (ok) {
        ...
        emit csyncError(tr("Không thể đọc danh sách đen từ cơ sở dữ liệu cục bộ"));
        finalize(false);
        return;
    }
...
void SyncEngine::slotDiscoveryJobFinished(int discoveryResult)
{
    ...
    if (!_journal->isConnected()) {
        ...
        emit csyncError(tr("Không thể mở nhật ký đồng bộ"));
        finalize(false);
        return;
    } else {
    ...
    if (_account->serverVersionInt() < 0x080100) {
        ...
        for (auto it = _syncedItems.begin(); it != _syncedItems.end(); ++it) {
            if ((*it)->_direction == SyncFileItem::Up &&
                ...
                (*it)->_errorString  = tr("Tên tệp chứa ít nhất một ký tự không hợp lệ");
...
void SyncEngine::slotItemCompleted(const SyncFileItem &item, const PropagatorJob &job)
{
    ...
    if (item._status == SyncFileItem::FatalError) {
		...
		//Nghia add
		emit csyncError(tr("Lỗi đồng bộ"));
		//Tên thư mục không được phép chứa ký tự đặc biệt
...
void SyncEngine::checkForPermission()
{
    ...
    for (SyncFileItemVector::iterator it = _syncedItems.begin(); it != _syncedItems.end(); ++it) {
        ...
        if (!selectiveListOk || std::binary_search(selectiveSyncBlackList.constBegin(), selectiveSyncBlackList.constEnd(),
            ...
            (*it)->_errorString = tr("Bỏ qua vì \"Chọn nội dung để đồng bộ hóa\" danh sách đen");
        ...
        switch((*it)->_instruction) {
            ...
            case CSYNC_INSTRUCTION_NEW: {
                ...
                if (perms.isNull()) {
                    ...
                    (*it)->_errorString = tr("Không được phép vì bạn không có quyền thêm thư mục con vào thư mục đó");
                    ...
                    for (SyncFileItemVector::iterator it_next = it + 1; it_next != _syncedItems.end() && (*it_next)->destination().startsWith(path); ++it_next) {
                        ...
                        (*it)->_instruction = CSYNC_INSTRUCTION_ERROR;
                        (*it)->_status = SyncFileItem::NormalError;
                        (*it)->_errorString = tr("Không được phép vì bạn không có quyền thêm thư mục gốc");
                    }
                    ...
                    (*it)->_instruction = CSYNC_INSTRUCTION_ERROR;
                    (*it)->_status = SyncFileItem::NormalError;
                    (*it)->_errorString = tr("Không được phép vì bạn không có quyền thêm tệp trong thư mục đó");
                }
                break;
            }
            case CSYNC_INSTRUCTION_SYNC: {
                ...
                if (!perms.contains("D")) {
                    ...
                    if( (*it)->_isDirectory ) {
                        ...
                        if( it == _syncedItems.begin() || !(path.startsWith((*(it-1))->_file)) ) {
                            (*it)->_errorString = tr("Đã xóa các tệp cục bộ và thư mục chia sẻ.");
                        }
            ...
            case CSYNC_INSTRUCTION_RENAME: {
                ...
                if (!sourceOK && (!destinationOK || isRename)
                    ...
                    (*it)->_direction = SyncFileItem::Down;
                    (*it)->_errorString = tr("Không cho phép di chuyển, khôi phục mục");
                    (*it)->_isRestoration = true;
                ...
                if (!sourceOK || !destinationOK) {
                    ...
                    (*it)->_instruction = CSYNC_INSTRUCTION_ERROR;
                    (*it)->_status = SyncFileItem::NormalError;
                    const QString errorString = tr("Di chuyển không được phép vì %1là chỉ đọc").arg(
                        sourceOK ? tr("đích đến") : tr("nguồn"));
