...
void GETFileJob::slotMetaDataChanged()
{
    ...
    if (!_directDownloadUrl.isEmpty() && !_etag.isEmpty()) {
        ...
        _errorString = tr("Không có E-Tag nhận được từ máy chủ, kiểm tra Proxy / Gateway.");
        ...
        _errorString = tr("Chúng tôi đã nhận được một Thẻ E-Tag khác để bắt đầu lại. Đang thử lại lần tiếp theo.");
        _errorStatus = SyncFileItem::NormalError;
        reply()->abort();
        return;
    }
    ...
    if (start != _resumeStart) {
        ...
        if (ranges.isEmpty()) {
            ...
            _errorString = tr("Máy chủ trả về sai phạm vi nội dung ");
            _errorStatus = SyncFileItem::NormalError;
            reply()->abort();
            return;
        }
...
void PropagateDownloadFileQNAM::start()
{
    ...
    if( _propagator->localFileNameClash(_item->_file) ) {
        done( SyncFileItem::NormalError, tr("Không thể tải lên tệp %1 vì sự cố đụng độ tập tin cục bộ!")
    ...
    if (diskSpaceResult == OwncloudPropagator::DiskSpaceFailure) {
        _item->_errorMayBeBlacklisted = true;
             ...
             tr("Việc tải xuống sẽ làm giảm không gian đĩa trống %1").arg(
        ...
        return;
             ...
             tr("Dung lượng đĩa trống còn lại %1").arg(
...
void PropagateDownloadFileQNAM::slotGetFinished()
{
    ...
    if (err != QNetworkReply::NoError) {
        ...
        if(!_item->_directDownloadUrl.isEmpty() && err != QNetworkReply::OperationCanceledError) {
            ...
            qWarning() << "Direct download of" << _item->_directDownloadUrl << "failed. Retrying through vnptdrive.";
        ...
        if (err == QNetworkReply::OperationCanceledError && reply->property(owncloudCustomSoftErrorStringC).isValid()) {
            ...
            job->setErrorString(tr("Tập tin đã xoa từ máy chủ"));
            job->setErrorStatus(SyncFileItem::SoftError);
        }
...
void PropagateDownloadFileQNAM::downloadFinished()
{
    ...
    if (_propagator->localFileNameClash(_item->_file)) {
        done( SyncFileItem::NormalError, tr("Không thể lưu tệp %1 vì có sự cố đụng độ tập tin cục bộ!")
              .arg(QDir::toNativeSeparators(_item->_file)) );
        return;
    }
    ...
    if (FileSystem::fileExists(fn)) {
        ...
        if (! FileSystem::verifyFileUnchanged(fn, expectedSize, expectedMtime)) {
            _propagator->_anotherSyncNeeded = true;
            done(SyncFileItem::SoftError, tr("Tập tin đã được thay đổi khi tìm thấy "));
            return;
        }
    }
    ...
    if (!_propagator->_journal->setFileRecord(SyncJournalFileRecord(*_item, fn))) {
        done(SyncFileItem::FatalError, tr("Lỗi khi ghi dữ liệu vào cơ sở dữ liệu"));
        return;
    }
    _propagator->_journal->setDownloadInfo(_item->_file, SyncJournalDb::DownloadInfo());
