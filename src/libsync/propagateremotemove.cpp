    ...
    if (_item->_file == QLatin1String("Shared") ) {
        ...
        if (versionString.contains('.') && versionString.split('.')[0].toInt() < 7) {
            ...
            if( FileSystem::rename(targetFile, originalFile, &renameError) ) {
                done(SyncFileItem::NormalError, tr("Không được đổi tên thư mục này. Nó được đổi tên thành tên ban đầu."));
            } else {
                done(SyncFileItem::NormalError, tr("Không được đổi tên thư mục này. Vui lòng đặt lại tên cho Shared"));
            }
            return;
        }
    }
...
void PropagateRemoteMove::slotMoveJobFinished()
{
    ...
    if (err != QNetworkReply::NoError) {
        ...
        if( checkForProblemsWithShared(_item->_httpErrorCode,
                tr("Tệp đã được đổi tên nhưng là một phần của chia sẻ. Đã khôi phục tệp ban đầu."))) {
            return;
        }
    ...
    if (_item->_httpErrorCode != 201 ) {
        ...
        // throw an error.
        done(SyncFileItem::NormalError, tr("Mã HTTP không đúng được trả về bởi máy chủ. Dự kiến ​​201, nhưng đã nhận được \"%1 %2\".")
