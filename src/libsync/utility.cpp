...
QByteArray Utility::userAgentString()
{
    ...
    if( appName != QLatin1String("VNPT DRIVE") ) {
...
QString Utility::toCSyncScheme(const QString &urlStr)
{
    ...
    if( url.scheme() == QLatin1String("http") ) {
        url.setScheme( QLatin1String("vnptdrive") );
    } else {
        // connect SSL!
        url.setScheme( QLatin1String("vnptdrive") );
    }
...
QString Utility::timeAgoInWords(const QDateTime& dt, const QDateTime& from)
{
    ...
    if( dt.daysTo(now)>0 ) {
        ...
        return QObject::tr("%n ngày trước", "", dtn);
    } else {
        qint64 secs = dt.secsTo(now);
        if( secs < 0 ) {
            return QObject::tr("trong tương lai");
        }
        if( floor(secs / 3600.0) > 0 ) {
            int hours = floor(secs/3600.0);
            return( QObject::tr("%n giờ trước", "", hours) );
        } else {
            int minutes = qRound(secs/60.0);
            if( minutes == 0 ) {
                if(secs < 5) {
                    return QObject::tr("bây giờ");
                } else {
                    return QObject::tr("Ít hơn một phút");
                }
            }
            return( QObject::tr("%n phút trước", "", minutes) );
        }
    }
    return QObject::tr("Một thời gian trước");
}
