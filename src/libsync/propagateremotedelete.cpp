...
void PropagateRemoteDelete::slotDeleteJobFinished()
{
    ...
    if (err != QNetworkReply::NoError && err != QNetworkReply::ContentNotFoundError) {
        ...
        if( checkForProblemsWithShared(_item->_httpErrorCode,
            tr("Tệp đã bị xoá khỏi chia sẻ.Đã được phục hồi.")) ) {
            return;
        }
    ...
    if (httpStatus != 204 && httpStatus != 404) {
        ...
        // throw an error.
        done(SyncFileItem::NormalError, tr("Mã HTTP không đúng được trả về bởi máy chủ. Dự kiến ​​204, nhưng đã nhận được\"%1 %2\".")
