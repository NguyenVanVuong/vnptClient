...
void PUTFileJob::slotTimeout() {
    ...
    if (!reply())
        return;
    _errorString =  tr("Quá thời gian kết nối");
    reply()->abort();
}
...
bool PollJob::finished()
{
    ...
    if (!ok || status.isEmpty()) {
        _item->_errorString = tr("Trả lời JSON không hợp lệ từ URL");
...
void PropagateUploadFileQNAM::slotStartUpload(const QByteArray& transmissionChecksumType, const QByteArray& transmissionChecksum)
{
    ...
    if (!FileSystem::fileExists(fullFilePath)) {
        done(SyncFileItem::SoftError, tr("Tệp tin đã xóa"));
        return;
    }
    ...
    if( prevModtime != _item->_modtime ) {
        ...
        done(SyncFileItem::SoftError, tr("Tệp cục bộ đã thay đổi trong quá trình đồng bộ hóa. Nó sẽ được tiếp tục."));
        return;
    }
    ...
    if (fileIsStillChanging(*_item)) {
        _propagator->_anotherSyncNeeded = true;
        done(SyncFileItem::SoftError, tr("Tập tin cục bộ đã thay đổi trong quá trình đồng bộ."));
        return;
    }
...
void PropagateUploadFileQNAM::slotPutFinished()
{
    ...
    if (err == QNetworkReply::OperationCanceledError && job->reply()->property(owncloudShouldSoftCancelPropertyName).isValid()) {
        ...
        _propagator->_anotherSyncNeeded = true;
        abortWithError(SyncFileItem::SoftError, tr("Buộc hủy công việc trên thiết lập lại kết nối HTTP với Qt < 5.4.2."));
        return;
    }
#endif
    ...
    if (err != QNetworkReply::NoError) {
        ...
        if(checkForProblemsWithShared(_item->_httpErrorCode,
            tr("Tệp đã được chỉnh sửa trên máy tính nhưng là một phần của chia sẻ chỉ đọc. "
               "Nó được khôi phục và chỉnh sửa của bạn nằm trong tệp xung đột."))) {
            return;
        }
    ...
    if (_item->_httpErrorCode == 202) {
        ...
        if (path.isEmpty()) {
            done(SyncFileItem::NormalError, tr("Thiếu URL"));
            return;
        }
        startPollJob(path);
        return;
    ...
    if (! FileSystem::verifyFileUnchanged(fullFilePath, _item->_size, _item->_modtime)) {
        _propagator->_anotherSyncNeeded = true;
        if( !finished ) {
            abortWithError(SyncFileItem::SoftError, tr("Tập tin cục bộ đã thay đổi trong quá trình đồng bộ."));
    ...
    if (!finished) {
        ...
        if (_currentChunk >= _chunkCount) {
            ...
            _finished = true;
            done(SyncFileItem::NormalError, tr("Máy chủ đã không thừa nhận đoạn cuối cùng. (Không có e-tag nào có mặt)"));
            return;
        }
...
void PropagateUploadFileQNAM::finalize(const SyncFileItem &copy)
{
    ...
    if (!_propagator->_journal->setFileRecord(SyncJournalFileRecord(*_item, _propagator->getFilePath(_item->_file)))) {
        done(SyncFileItem::FatalError, tr("Lỗi khi ghi siêu dữ liệu vào cơ sở dữ liệu"));
        return;
    }
